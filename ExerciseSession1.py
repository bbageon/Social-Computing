...
# -----------------Exercise 2-------------------
tablerows2_df = pd.read_sql_query("SELECT location, count(id) as user_count from users where location IS NOT NULL and location != ' ' group by location order by 2 DESC Limit 5;", con)
# print(tablerows2_df)
...
# ---------- Exercise 4 -----------------
tablerows4_df = pd.read_sql_query("SELECT  id, count(followed_id) as follower_number from follows join users on id = followed_id group by id order by follower_number DESC LIMIT 5;", con)
print(tablerows4_df)
